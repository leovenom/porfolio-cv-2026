...
export type SplashCursorProps = {
  className?: string
  /** Element that receives pointer events (e.g. the hero section). Canvas stays visual-only. */
...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    let pointers: FluidPointer[] = [createPointer()]
    ...
    function updatePointerMoveData(pointer: FluidPointer, posX: number, posY: number, color: RGB) {
      pointer.prevTexcoordX = pointer.texcoordX
      pointer.prevTexcoordY = pointer.texcoordY
      pointer.texcoordX = posX / canvas.width
      pointer.texcoordY = 1.0 - posY / canvas.height
    ...
    function handleMouseDown(e: MouseEvent) {
      const { x, y, inside } = getCanvasPointer(e.clientX, e.clientY)
      if (!inside) return
    ...
    function handleTouchStart(e: TouchEvent) {
      ...
      for (let i = 0; i < touches.length; i++) {
        const { x, y, inside } = getCanvasPointer(touches[i].clientX, touches[i].clientY)
        if (!inside) continue
    ...
    return () => {
      ...
      interactionTarget.removeEventListener('mousedown', handleMouseDown)
